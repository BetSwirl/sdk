...
> **Quick start:** Download [`.env.example`](https://github.com/BetSwirl/miniapp-ui-react-demo/blob/main/.env.example), copy it to `.env`, replace only `NEXT_PUBLIC_URL` with your Vercel domain, and upload the entire file to Vercel. All other variables have working defaults or can be filled later.
