...
**Implementation:** Since `setGame` is inside App component and `LeaderboardProvider` is in providers.tsx, you need to pass the callback down or lift state up. Here's a simple approach using `console.log` for now:
