...
**Required variables (will be public in manifest):**
```bash
...
These optional images are used in manifest metadata:
...
```bash
# Manifest metadata (will be public in manifest)
...
# BetSwirl affiliate address (not in manifest)
# Earn 30% commission from bets
NEXT_PUBLIC_AFFILIATE_ADDRESS=

# Custom RPC endpoints (not in manifest)
