...
#### Step 7: Leaderboard Integration

**Learn more:** [Leaderboard Integration](../configuration#leaderboard-integration)

**Implementation:** Since `setGame` is inside App component and `LeaderboardProvider` is in `src/main.tsx`, you need to pass the callback down or lift state up. Here's a simple approach using `console.log` for now:
...
For detailed information about all `BetSwirlSDKProvider` parameters, see the [Configuration Reference](../configuration).
